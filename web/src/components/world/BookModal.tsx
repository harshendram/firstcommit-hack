...
/**
 * Only a true phone-width pane goes single-leaf. A split desktop window is
 * often 600–750px — under the old 760 cutoff StPageFlip dropped to portrait
 * and the tome read as a floating card, not a book. A spread is 640px; we
 * keep that layout and scale the stage down to fit, rather than collapsing.
 */
const SPREAD_MIN_WIDTH = 520;

function fitScale(portrait: boolean) {
  if (typeof window === "undefined") return 1;
  const needW = portrait ? PAGE_W : PAGE_W * 2;
  const w = Math.max(window.innerWidth - 48, 160);
  const h = Math.max(window.innerHeight - 48, 160);
  return Math.min(1, w / needW, h / PAGE_H);
}
...
const flipSettings = (portrait: boolean) =>
  ({
    ...
    startZIndex: 0,
    // If this is true, a narrow parent makes StPageFlip re-portrait the book
    // even when we asked for a spread.
    autoSize: false,
...
function Tome({ onClose }: { onClose: () => void }) {
  ...
  const [fit] = useState(() => fitScale(portrait));
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        initial={{ opacity: 0, scale: fit * 0.94, y: 24 }}
        animate={{ opacity: 1, scale: fit, y: 0 }}
        exit={{ opacity: 0, scale: fit * 0.97, y: 12 }}
